...
public class Graph {
	...
	// the neighboring vertices with name and cost
	public class Vertex{
		HashMap<String,Integer> ngbr = new HashMap<>();
	}
	
     // Name of every vertex along with its neighboring vertices and the cost	
	...
	// might be the number of neighbors of each vertices
	public int numVertex() {
		return this.vertices.size();
	}
	// returns weather a particular vertex has a particular neighbor or not
	...
	public void removeVertex(String vname) 
	{
		...
		ArrayList<String> keys = new ArrayList<>(vtx.ngbr.keySet());

		for (String key : keys) 
		{
			Vertex nbrVtx = vertices.get(key);
			nbrVtx.ngbr.remove(vname);
		}
	...
	public int numEdges() {
		...
		for(String key:keys) {
			Vertex vtxVertex = vertices.get(key);
			count += vtxVertex.ngbr.size();
		}
	...
	public void addEdge(String vname1, String vname2, int value) 
	{
		...
		vtx1.ngbr.put(vname2, value);
		vtx2.ngbr.put(vname1, value);
	}
	...
	public void display_Map() {
		...
		for(String key:keys) {
			...
			ArrayList<String> nbr = new ArrayList<>(vert.ngbr.keySet());
			for(String nbrs:nbr) {
				        string = string + "\t" + nbrs + "\t";
				        if(nbrs.length()<16) {
					    string = string+"\t";
					    if(nbrs.length()<8) {
						string = string + "\t";
						string = string + vert.ngbr.get(nbrs)+"\n";
	...
	public int dijkstra(String src,String des,boolean nn) {
		int val=0;
		...
		while(!heap.isEmpty()) {
			DPair dp = heap.remove();
			if(dp.nameString.equals(des)) {
				val = dp.cost;
				break;
			}
			
			map.remove(dp.nameString);
			anStrings.add(dp.nameString);
			
			Vertex vert = vertices.get(dp.nameString);
			for(String nbr:vert.ngbr.keySet()) {
				if(map.containsKey(nbr)) {
					int oc = map.get(nbr).cost;
					Vertex k = vertices.get(dp.nameString);
					int nc;
					if(nn) {
						// time
						nc = dp.cost + 120 + 40*k.ngbr.get(nbr);
					}else {
						// distance
						nc = dp.cost + k.ngbr.get(nbr);
					}
	...
	public boolean hasPath(String vname1, String vname2, HashMap<String, Boolean> processed) 
	{
		...
		ArrayList<String> nbrs = new ArrayList<>(vtx.ngbr.keySet());
	...
	public String Get_Minimum_Distance(String src, String dst) 
	{
		...
		while (!stack.isEmpty()) 
		{
			...
			if (processed.containsKey(rp.nameString)) 
			{
				continue;
			}

			// processed put
			processed.put(rp.nameString, true);
			
			//if there exists a direct edge b/w removed pair and destination vertex
			if (rp.nameString.equals(dst)) 
			{
				int temp = rp.min_d;
				if(temp<min) {
					ans = rp.psf;
			...
			Vertex rpvtx = vertices.get(rp.nameString);
			ArrayList<String> nbrs = new ArrayList<>(rpvtx.ngbr.keySet());
			...
			for(String nbr : nbrs) 
			{
				...
				if (!processed.containsKey(nbr)) {
					...
					np.nameString = nbr;
					np.psf = rp.psf + nbr + "  ";
					np.min_d = rp.min_d + rpvtx.ngbr.get(nbr); 
		...
		while (!stack.isEmpty()) {
			...
			for (String nbr : nbrs) 
			{
				...
				if (!processed.containsKey(nbr)) {
					...
					np.min_t = rp.min_t + 120 + 40*rpvtx.ngbr.get(nbr); 
					stack.addFirst(np);
				}
			}
		}
	...
	{
		g.addVertex("Noida Electronic City~B");
		...
		g.addEdge("Noida Electronic City~B", "Botanical Garden~B", 9);
		g.addEdge("Botanical Garden~B", "Yamuna Bank~B", 9);
